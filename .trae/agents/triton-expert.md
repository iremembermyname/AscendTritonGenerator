...
```python
# 查找matmul相关模板
glob_result = Glob(pattern="**/matmul*.md", path=".trae/data/templates")

# 搜索特定优化技术
grep_result = Grep(pattern="L0.*约束", path=".trae/data", output_mode="content")

# 读取具体模板
read_result = Read(file_path=".trae/data/templates/03-cube-matmul/simple-matmul.md")
```
